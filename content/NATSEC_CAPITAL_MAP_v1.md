...
- OSC's full 31-category list is assembled from a partial secondary source, not the statute or OSC's own Investment Strategy directly; the complete list still needs a primary-source pull. The competing 34 figure most likely describes the pilot program specifically rather than the full statutory authority, a different scope rather than a contradiction, but that reading itself is unconfirmed.
...
- National Defense Stockpile: the exact division of labor between direct DLA purchases and the US SIIE, LLC special-purpose-vehicle structure used on the largest recent deals is not confirmed.
- NextFlex's REARM special call: award size has now been searched for directly twice across two revisions and still isn't publicly posted; nextflex.us is the place to check before citing a figure.
- RIF-RISE's roughly 5% hit rate comes from a secondary source, not USD(A&S) reporting directly.
- MCWL: the Marine Innovation Unit's relationship to MCWL, and whether it does any company-facing funding itself, is unconfirmed.

**Service on-ramps and allied doors**
- NavalX's Tech Bridge count is confirmed at 18 sites, but the full 18-site list is still partial; confirm the complete roster at navalx.navy.mil before citing it.
- Navy's $30M SBIR/STTR broad-agency-announcement program: confirmed to exist through a Department of the Navy release, but its exact current name and solicitation status weren't pinned down against a live BAA this revision.
...
- AUKUS Pillar II: the Excluded Technology List's actual current scope remains undisclosed even though AUKUS defense ministers publicly committed to narrowing it in May 2026. A US entity has now won through the DIU leg of the Innovation Challenge (Florida Atlantic University, $1M, Maritime Innovation Challenge), which confirms the pathway pays out, but FAU is a university, not a startup, so whether a US company specifically can win through it, and on what terms, is still unconfirmed. The proposed multi-sovereign private-capital fund remains a proposal, not a live vehicle.

**Coordination bodies (section 5)**
- National Security Capital Forum: meeting cadence and concrete outputs to date are not confirmed.
- National Energy Dominance Council: membership runs to at least 19 officials by executive order, but the full current roster beyond the named chair, vice-chair, and a few named members is not confirmed.
- Minerals Security Partnership Finance Network: the cited $600M potential Dubbo rare-earths debt package's deal status and final terms are not confirmed.
...
**SBIR/STTR scope**
This map deprioritises SBIR and STTR by design. Army, Navy, and SOCOM service on-ramps got a deeper pass across two revisions; Air Force, Space Force, and civil-agency SBIR/STTR are still named only as an on-ramp, not inventoried in full. The Missile Defense Agency was checked separately and its SBIR/STTR program isn't distinct enough from the generic "service SBIR/STTR" bucket to warrant its own entry.
...
- The sector pathway grid applies two rules beyond literal name-matching: every physical-product sector (Hardware, Space, Marine) inherits the same six spine vehicles even where a sector's own paragraph doesn't re-name all of them, and EXIM and NSF are lit wherever their own directory entries name a sector, not only where that sector's paragraph happens to cite them.
