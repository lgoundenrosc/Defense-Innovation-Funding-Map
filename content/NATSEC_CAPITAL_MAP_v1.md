...
17 entries across 17 states, covering the states found across three research passes plus two (California, Florida) pulled in once the section's scope widened from defense-specific programs to state-run technology and innovation capital generally. This remains explicitly out of scope for the rest of the map's analytical apparatus, none of its entries feed the capital map matrix or the sector pathway grid, and it is a representative sample, not a 50-state census; a state's absence means it wasn't checked. Every entry in this section carries a heavier UNVERIFIED load than the rest of the document, mostly current deployment figures and fund balances that are inherently harder to pin down at a state level than a single federal program's terms.
...
Given that, this section stops well short of a 50-state inventory. It covers the states found in earlier passes (Massachusetts, Texas, New York, Illinois, Utah), the large-economy states asked about directly in two follow-up passes (Virginia, Ohio, Michigan, Washington, Georgia, North Carolina, Arizona, Pennsylvania, and the smaller-economy Maine and Connecticut), and two more (California, Florida) that the broadened lens pulled in once general SSBCI equity programs, not just defense-specific ones, were in scope. A state's absence from this list means it wasn't checked, not that it lacks a program; treat this as a representative sample, not a census.
